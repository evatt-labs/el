// Package naming ports kraai 0.5.0's JS naming derivation
// (legacy-node/src/names.mjs) to Go, byte-for-byte in behavior per
// docs/BLUEPRINT.md D22 — changing any of these orphans every environment
// already deployed by 0.4.x/0.5.x, independent of the language rewrite.
//
// It also owns two things the JS implementation never had:
//
//   - The persistent environment name grammar (D22), distinct from the
//     frozen ephemeral one.
//   - Import-reference resolution (D7): given a
//     resources.<service>.<kind>.<binding> entry in an environment
//     overlay (internal/manifest's ResourceImports/ImportRef), the
//     declared { id | name } already *is* that resource's identity — no
//     further derivation happens. This package only validates that
//     exactly one of the two is set.
//
// Resource identity *lookup* strategy (byName/byApi/byAttr/byTag, D26) and
// the identity cache (D27) are out of scope entirely: this package owns
// name derivation, not lookup, and never conflicts with either decision.
package naming
