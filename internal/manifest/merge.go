...
// (D4). sources names each file for error messages and must be the same
// length as files.
//
// A service key defined in two different files is a validation error, not
// last-file-wins: D4's whole rationale for a directory of small files
// instead of one monolith is PR reviewability, and a silent overwrite
// across files is exactly the kind of thing a reviewer could miss.
