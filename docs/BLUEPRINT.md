...
4. **D7's deterministic-name identity does not hold for all AWS resources — BLOCKING `aws-provider-core`.** Measured against the live CloudFormation registry on 2026-09-13: `primaryIdentifier` is name-derivable for `AWS::S3::Bucket` (BucketName), `AWS::Lambda::Function` (FunctionName), `AWS::SSM::Parameter` (Name) and `AWS::IAM::Role` (RoleName) — D7 works exactly as written for those. But `AWS::CloudFront::Distribution` and `AWS::Route53::HostedZone` use `/properties/Id`, and `AWS::CertificateManager::Certificate` uses `/properties/CertificateArn`: server-assigned values that cannot be recomputed from a manifest path. D6 forbids a state file, so there is nowhere to persist them either. Three candidate resolutions, none yet chosen: (a) **list-and-filter on a deterministic secondary attribute** — CloudFront `Comment` or `Aliases`, ACM `DomainName`, Route53 HostedZone `Name` — which keeps D6/D7/D9 intact but is O(n) per lookup and depends on that attribute being reliably unique; (b) **promote tags to load-bearing identity for these types**, which directly reverses D9's "tags are cosmetic, never load-bearing" and its cross-provider-uniformity reasoning; (c) **write the assigned ID back into the manifest** the way an import reference is written, which makes `apply` mutate a human-authored, git-resident file. This must be resolved before any Tier 1 resource code is written.
