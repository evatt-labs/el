...
| # | Decision | Why |
|---|----------|-----|
| D1 | Manifests are YAML, via `yaml` (2.x, ISC, zero deps, pinned exact). External dependencies are no longer capped at one: added when a vetted, actively-maintained package clearly earns its place — most concretely, a real schema validator (D17's generated schemas) or a maintained SigV4 signer, over hand-rolling either. Still weighed against dependency count as its own security surface for a CLI that holds cloud credentials (D2 trust model): fewer, well-known, pinned-exact packages, not "whatever's convenient." | Persistent envs are read and edited by humans and review tools; JS config can't be diffed or validated by anything but Node. D17's Terraform-grade schema coverage and D5/D18's generic S3-compatible signing are real enough surfaces that hand-rolling them was always a to-be-revisited choice, not a permanent one. |
